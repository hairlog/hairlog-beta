...
async function savePhrasesToDB(designerId: string, phrases: Record<string, string[]>) {
  const { supabase } = await import('../../lib/supabase')
  await supabase.from('designers').update({ custom_phrases: phrases }).eq('id', designerId)
}
...
export default function NewService() {
    ...
    import('../../lib/supabase').then(({ supabase }) => {
      supabase.from('designers').select('*').eq('id', s.designer_id).single()
        .then(({ data }) => {
          if (data) {
            setDesigner(data)
            if (data.custom_phrases) {
              setMyPhrases(data.custom_phrases)
            }
          }
        })
    })
  ...
  function savePhrase(tab: string) {
    const v = noteInput[tab]?.trim()
    if (!v) return
    const mine = [...(myPhrases[tab] || [])]
    if (mine.includes(v)) { alert('이미 저장된 문구예요'); return }
    mine.unshift(v); if (mine.length > 15) mine.pop()
    const updated = { ...myPhrases, [tab]: mine }
    setMyPhrases(updated)
    setNoteInput(prev => ({ ...prev, [tab]: '' }))
    if (session?.designer_id) savePhrasesToDB(session.designer_id, updated)
  }
  ...
  function handlePhotoSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    toAdd.forEach(file => {
      ...
      reader.onload = ev => {
        setPhotos(prev => [...prev, { preview: ev.target?.result as string, file }])
      }
  ...
  async function handleSubmit() {
    ...
    const res = await fetch('/api/service-records/create', {
      ...
      body: JSON.stringify({
        ...
        service_date: svcDate, service_type: buildSvcSummary(),
        gender: gender || null,
        ...
        <div className="card p-0 overflow-hidden">
          ...
          <div className="divide-y divide-gray-100">
                    ...
                    <div className="bg-gray-50 border-t border-gray-100 px-4 py-3 space-y-3">
                      {k === 'cut' && (gender === 'male' ? (
                        <div className="flex flex-wrap gap-2">
                          {CUT_MALE.map(t => <Chip key={t} label={t} active={curTags.includes(t)} onClick={() => toggleTag(k, t)} />)}
                        </div>
                      ) : !cutLen ? (
                        <div>
                          ...
                          <div className="flex gap-2 flex-wrap">
                            {['숏','단발','미디움','롱'].map(l => (
                              <button key={l} type="button" onClick={() => setCutLen(l)}
                                className="px-4 py-2 rounded-full text-sm font-semibold border border-gray-200 bg-white text-gray-600 hover:border-accent transition">
                                {l}
                              </button>
                            ))}
                          </div>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {(CUT_FEMALE[cutLen] || []).map(t => <Chip key={t} label={t} active={curTags.includes(t)} onClick={() => toggleTag(k, t)} />)}
                          </div>
                        </div>
                      ))}
                      {k === 'perm' && (
                        <div className="flex flex-wrap gap-2">
                          {(gender === 'male' ? PERM_MALE : PERM_FEMALE).map(t => <Chip key={t} label={t} active={curTags.includes(t)} onClick={() => toggleTag(k, t)} />)}
                        </div>
                      )}
                      {k === 'color' && (
                        <div className="space-y-3">
                          {Object.entries(COLOR_GROUPS).map(([grp, items]) => (
                            <div key={grp}>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {items.map(t => <Chip key={t} label={t} active={(colorTags[grp]||[]).includes(t)} onClick={() => toggleColorTag(grp, t)} />)}
                              </div>
                            </div>
                          ))}
                        </div>
                      ...
                      {k === 'clinic' && (
                        <div className="flex flex-wrap gap-2">
                          {CLINIC_LIST.map(t => <Chip key={t} label={t} active={curTags.includes(t)} onClick={() => toggleTag(k, t)} />)}
                        </div>
                      )}
                      {k === 'scalp' && (
                        <div className="flex flex-wrap gap-2">
                          {SCALP_LIST.map(t => <Chip key={t} label={t} active={curTags.includes(t)} onClick={() => toggleTag(k, t)} />)}
                        </div>
                      )}
                      {myStyles.length > 0 && (
                          ...
                          {myStyles.map((s: string, i: number) => (
                            <div key={i} className="flex">
                              <button type="button" onClick={() => toggleTag(k, s)}
                                className={`px-3 py-1.5 rounded-l-full text-xs font-semibold border-y border-l transition ${curTags.includes(s) ? 'bg-accent text-white border-accent' : 'bg-amber-50 text-amber-700 border-amber-300'}`}>
                                ⭐ {s}
                              </button>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button type="button" onClick={() => saveMyStyle(k)}
                          className="px-3 py-1.5 bg-accent text-white rounded-lg text-xs font-bold whitespace-nowrap">저장 ⭐</button>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="flex gap-2 ml-auto">
                {['상','중','하'].map(l => (
                  <button key={l} type="button" onClick={() => setDmgLevel(dmgLevel === l ? '' : l)}
                    className={`px-4 py-1.5 rounded-full text-xs font-bold border transition ${dmgLevel === l ? 'bg-primary text-white border-primary' : 'bg-white text-gray-500 border-gray-300'}`}>
                    {l}
                  </button>
                ))}
              </div>
            ...
            <div className="px-4 pb-3 bg-white">
              <input className="input text-sm" value={dmgPart} onChange={e => setDmgPart(e.target.value)}
                placeholder="부분 손상도 직접 입력 (예: 끝머리 20cm 가량)" />
            </div>
          </div>
        </div>
            ...
            {photos.map((p, i) => (
              <div key={i} className="relative">
                ...
                <button type="button" onClick={() => removePhoto(i)}
                  className="absolute -top-2 -right-2 w-5 h-5 bg-red-500 text-white rounded-full text-xs flex items-center justify-center font-bold shadow">×</button>
              </div>
            ))}
            {photos.length < 5 && (
        ...
        <div className="card p-0 overflow-hidden">
          <div className="flex border-b border-gray-100">
            {noteTabs.map(t => (
              <button key={t.k} onClick={() => setNoteTab(t.k)}
                className={`flex-1 py-3 text-xs font-semibold border-b-2 transition ${noteTab === t.k ? 'text-primary border-primary' : 'text-gray-400 border-transparent'}`}>
                {t.l}
              </button>
            ))}
          </div>
          <div className="p-4 space-y-3">
                ...
                {(myPhrases[noteTab] || []).map((p: string, i: number) => (
                  <div key={i} className="flex">
                    <button type="button" onClick={() => addPhrase(noteTab, p)}
                      className="px-3 py-1.5 rounded-l-full text-xs font-semibold bg-amber-50 text-amber-700 border-y border-l border-amber-300 hover:bg-amber-100 max-w-[200px] truncate">
                      ⭐ {p}
                    </button>
            ...
            <div className="flex flex-wrap gap-2">
              {(DEFAULT_PHRASES[noteTab] || []).map((p: string) => (
                <button key={p} type="button" onClick={() => addPhrase(noteTab, p)}
                  className="px-3 py-1.5 rounded-full text-xs font-medium border border-gray-200 bg-white text-gray-600 hover:bg-gray-50 transition">
                  {p}
                </button>
              ))}
            </div>
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => savePhrase(noteTab)}
                className="px-3 py-2 bg-accent text-white rounded-lg text-xs font-bold whitespace-nowrap">저장 ⭐</button>
            </div>
        ...
        <div className="card p-0 overflow-hidden divide-y divide-gray-100">
          <div className="px-4 py-3 flex items-center justify-between">
            ...
            <button onClick={() => setShowRecipe(!showRecipe)}
              className={`w-11 h-6 rounded-full transition-colors relative ${showRecipe ? 'bg-primary' : 'bg-gray-200'}`}>
              <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${showRecipe ? 'left-5' : 'left-0.5'}`} />
            </button>
          </div>
          {showRecipe && (
            <div className="px-4 pb-3">
              <textarea className="input text-sm mt-2 resize-none" rows={2} value={recipe} onChange={e => setRecipe(e.target.value)}
                placeholder="예) 밀본 7.60 × 3% 산화제 1:1, 방치 25분" />
            </div>
          )}
          <button type="button" onClick={() => setShowMemo(!showMemo)}
          ...
          {showMemo && (
            <div className="px-4 pb-3">
              <textarea className="input text-sm resize-none" rows={2} value={custMemo} onChange={e => setCustMemo(e.target.value)}
                placeholder="예) 다음엔 파마 강도 조금 세게, 두피 민감" />
            </div>
          )}
        </div>
